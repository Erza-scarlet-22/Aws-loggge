...
# AWS mode config (read once at import time)
...
def _s3_put_log_lines(lines: list):
    ...
    try:
        ...
        key  = f"{_RAW_PREFIX}{dt}/dummy-app-{ts}.log"
        body = '\n'.join(lines) + '\n'
        boto3.client('s3', region_name=_AWS_REGION).put_object(
            Bucket=_RAW_BUCKET, Key=key,
            Body=body.encode('utf-8'), ContentType='text/plain')
        _log.info('[dummy_app] S3 upload OK: s3://%s/%s', _RAW_BUCKET, key)
