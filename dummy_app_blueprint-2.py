...
# Keep backward-compat names for existing code
_RAW_BUCKET = property(lambda self: _get_raw_bucket())
_RAW_PREFIX = property(lambda self: _get_raw_prefix())
_AWS_REGION = property(lambda self: _get_aws_region())
_IS_AWS     = property(lambda self: _is_aws())
...
def _s3_put_log_lines(lines: list):
    ...
    try:
        ...
        _log.info('[dummy_app] S3 upload OK: s3://%s/%s', raw_bucket, key)
    except Exception as exc:
        _log.error('[dummy_app] S3 upload failed: %s', exc)
